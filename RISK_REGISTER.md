...
| R5 | Cross-target artifacts may regress silently in packaging flow | Medium | Medium | Matrix build script + package sync script + staged prebuilds + optional-install smoke script in CI/release workflows | Add remote workflow assertions for platform package publish metadata before npm release |
